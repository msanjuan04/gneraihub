...
import type { Mensualidad, Client, Currency } from "@/types";
...
export default async function ProyectoDetailPage({ params }: Props) {
  ...
  const [projectRes, invoicesRes, expensesRes, incomeRes, mensualidadesRes, clientsRes] =
    await Promise.all([
  ...
  const mrr = activeMensualidades.reduce(
  ...
  // Proyectos tienen mensualidades → mostrar KPIs extra
  const hasMensualidades = mensualidades.length > 0;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex-1">
          ...
          {hasMensualidades && (
            <p className="text-sm text-muted-foreground">
              ...
              {mrr > 0 && (
                <> · MRR: <span className="text-income font-medium">{formatCurrency(mrr, project.currency)}</span></>
              )}
            </p>
          )}
        </div>
          ...
          <>
            <Card>
              <CardContent className="pt-6">
                <p className="text-xs text-muted-foreground">Mensualidades activas</p>
                <p className="text-lg font-bold">{activeMensualidades.length}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <p className="text-xs text-muted-foreground">MRR</p>
                <p className="text-lg font-bold text-income">{formatCurrency(mrr, project.currency)}</p>
              </CardContent>
            </Card>
          </>
        ) : (
      ...
      <Tabs defaultValue="mensualidades">
        ...
        {/* Tab Mensualidades — disponible en TODOS los proyectos */}
